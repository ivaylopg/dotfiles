...
declare -a DIRS_TO_SYMLINK=$(find . -type d -maxdepth 1 -name ".*" -not -name .DS_Store -not -name .git -not -name .osx -not -name .config -not -name . | sed -e 's|//|/|' | sed -e 's|./.|.|')
DIRS_TO_SYMLINK="$DIRS_TO_SYMLINK"

declare -a CONFIG_DIRS_TO_SYMLINK=$(find ./.config/ -type d -maxdepth 1 -name "*" -not -name .DS_Store -not -name .git -not -name .osx -not -name .config -not -name . | sed -e 's|//|/|' | sed -e 's|./.|.|')
CONFIG_DIRS_TO_SYMLINK="$CONFIG_DIRS_TO_SYMLINK"
...
main() {
    ...
    for i in ${CONFIG_DIRS_TO_SYMLINK[@]}; do
